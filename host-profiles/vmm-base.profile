# Base Firejail policy for host-side VMM processes.
#
# This is intentionally different from a normal coding-agent profile: VMMs need
# /dev/kvm, so this profile avoids private-dev and instead blocks common host
# device/session escape surfaces explicitly. The runner or test harness adds
# --whitelist=$PROJECT_DIR so VM images, sockets, and logs should live there.

include vmm-base.local
...
# Do not expose host desktop/session buses or container control sockets.
...
# Do not expose raw host storage. VMM disk images should be project-local files.
blacklist /dev/sd*
blacklist /dev/nvme*
blacklist /dev/vd*
