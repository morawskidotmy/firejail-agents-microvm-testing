# Host Firejail policy for Firecracker.
# Keep this profile self-contained so it can be passed directly to firejail.

include firecracker-host.local

whitelist ${HOME}/.local/bin/firecracker
whitelist ${HOME}/.local/bin/jailer
read-only ${HOME}/.local/bin/firecracker
read-only ${HOME}/.local/bin/jailer
